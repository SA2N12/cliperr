...
export const api = {
  ...
  // Reprend les niches deja saisies sur les comptes et en fait des fiches.
  importNiches: () => post<{ ok: boolean; crees: number; assignes: number }>('/api/niches/import'),
  ...
  saveAutopilotSlot: (slot: { user: string; ordinal: number; hm?: number | null; type?: string | null; subject?: string | null; music?: string | null; stockPick?: string | null; stockKinds?: string | null; reset?: boolean; day?: number }) =>
  ...
  generateIdeaVideo: (id: number, lang?: 'fr' | 'en', quality?: 'eco' | 'wan' | 'seedance' | 'veo') =>
  ...
  deepinfraStatus: () => req<{ has: boolean }>('/api/settings/deepinfra'),
  setDeepinfraKey: (key: string) => post('/api/settings/deepinfra', { key }),
